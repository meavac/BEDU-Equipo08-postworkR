...
# Leemos los datos y los agregamos a una variable
...
x <- sample(c(0:5),1,F)
y <- sample(c(0:6),1,F)
paste("La probabilidad de que el equipo que juega como visitante anote", x, "goles es de",
      round(data.goals.table[8, x+1]*100,4),"%")
paste("La probabilidad de que el equipo que juega en casa anote", y, "goles es de",
      round(data.goals.table[y+1,7]*100,4),"%")
